...
- **AI Gateway**: `anthropic/*` chat models run through the gateway's **native Anthropic passthrough** (`.../anthropic`, see `createAnthropicClient` in `src/lib/llm.ts`) — required for prompt caching (`cache_control` breakpoints; rounds 2+ read the prefix at 0.1× cost) and for streaming tool-arg deltas (progressive canvas preview; needs the `fine-grained-tool-streaming` beta header). Other models use the OpenAI-compat unified endpoint, which supports neither — don't move anthropic models back to it. The `AI` binding (Workers AI) is reserved for cheap background tasks like chat-title generation. `scripts/stream-repro.sh` drives a full turn against a local worker and prints per-poll timing + per-patch timestamps — use it to verify streaming/caching changes (check `round_end` logs for `cacheRead`).
